...
if [[ ! -f deploy/.env.prod ]]; then
  echo "==> 生成 deploy/.env.prod（请编辑填入 ZHIPU_API_KEY，可选）"
  SECRET=$(openssl rand -hex 24)
  ADMIN_PASS=$(openssl rand -hex 12)
  PG_PASS=$(openssl rand -hex 16)
  ZHIPU_KEY=""
  if [[ -f backend/.env ]]; then
    ZHIPU_KEY=$(grep -E '^ZHIPU_API_KEY=' backend/.env 2>/dev/null | cut -d= -f2- || true)
  fi
...
PORT=3001
EOF
  echo "⚠️  可选：编辑 deploy/.env.prod 填入 ZHIPU_API_KEY（未填则纯本地解读，不报错）"
  echo "🔑  管理后台密码已写入 deploy/.env.prod 的 ADMIN_PASSWORD（访问 /admin）"
fi

if ! grep -q '^ZHIPU_API_KEY=.\+' deploy/.env.prod 2>/dev/null; then
  echo "ℹ️  deploy/.env.prod 未配置 ZHIPU_API_KEY，上线后使用纯本地解读（无 AI 润色）"
fi
